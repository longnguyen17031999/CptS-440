...
class SearchState:
	...
	def __eq__(self, other): #operator overloading for ==
...
class SearchEngine:
	...
	def AStarSearch (self, initialState, goalState):
		initialState.heuristic = self.HeuristicFunction(initialState, goalState)
		...
		while (self.frontier):
			...
			for action in [Action.GOFORWARD, Action.TURNLEFT, Action.TURNRIGHT]:
				...
				if (childState):
					self.nodeCount += 1
					childState.heuristic = self.HeuristicFunction (childState, goalState)
